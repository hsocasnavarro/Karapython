...
"""
    ...
    bpm=[[120,0.]] # bpm using actual time signature
    microsecondsperquarternote=[[60000000./120,0.]]
    num=[[4, 0.]]
    den=[[4, 0.]]
    # For karaoke .kar files
    karfile=False
    ...
    def read_var_length(self):
        ...
        return [var,iread] # Return value and number of bytes read


    def load_file(self,fileobject):
        
        if type(fileobject) == str:
            self.fileobject=open(fileobject)
        ...
        for itrack in range(self.ntracks):
            ...
            while iread < tracklen and metatype != 0x2f:
                [dtime,nbytesread]=self.read_var_length()
                ...
                if status == 0xFF: # It's a non-MIDI event, META event
                    ...
                    if status1 == 0b1100: # Program change
                        read=struct.unpack('>B',self.fileobject.read(1))[0]
                        currentpatch=read
                        self.patchesused.append([itrack,currentpatch])
                        iread=iread+1
